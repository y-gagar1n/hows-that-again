...
Поэтому результат операции хранится снаруже футуры и промиса, в локации под названием *shared state*:
...
- деструктор последней футуры, ссылающеся на разделяемое состояние для не-отложенной таски, запущенной через `std::async` блочится, пока таска не завершится.
- деструкторы остальных футур просто уничтожают футуры